...
/**
 * Weighted Product Method — FIXED VERSION
 * Cocok untuk nilai skala 1–5
 */
...
export function weightedProduct(alternatifs, bobot, tipe) {
  ...
  const hasil = alternatifs.map((alt) => {
    ...
    for (let i = 0; i < w.length; i++) {
      let nilai = Number(alt.nilai[i]);

      // Validasi nilai
      if (isNaN(nilai) || nilai <= 0) continue;

      // FIX: cost harus dibalik
      if (tipe[i] === "cost") {
        nilai = 1 / nilai;
      }

      score *= Math.pow(nilai, w[i]);
    }
  ...
  const totalScore = hasil.reduce((sum, h) => sum + h.score, 0);
